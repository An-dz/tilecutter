...
class DebugFrame(wx.Frame):
    ...
    def __init__(self, parent, log, id=wx.ID_ANY, title=""):
        ...
        self.textbox = wx.TextCtrl(self.panel, wx.ID_ANY, self.log.stringio.getvalue(), (-1,-1), (-1,-1), wx.TE_MULTILINE|wx.TE_READONLY)
    ...
    def update(self):
        ...
        self.count += 1
        val = self.log.stringio.getvalue()
        self.textbox.SetValue(val + str(self.count))
        self.textbox.ShowPosition(len(val))
        self.timer.enter(10, 1, self.update, ())
        self.timer.run()
