...
# Translation system - On the fly translation system now possible
...
# Add the TileCutter icon   - Icon made
# Use img2py to compile icon image into the application (for built-in icon provision, e.g. displayed in the program,
#   for the icon displayed in explorer/finder etc. this should be included seperately into the exe/app bundle)
# Can use the same technique for all the other images, e.g. bitmap buttons etc.
...
def gtm(menuID, a=None):
    """Get text for menus, takes a menu identifier and returns translated text and the user's mapped control key"""
    # Remember, check input string from translation for \t's and remove them & everything to the right of them
    # to prevent translators from messing with shortcuts
    return menuID + "\t" + a
    # This should lookup the control keys based on the menuID and then combine them together, so that the user can define
    # the control keys seperate from the translation being used (so they can be consistent across translations)
    # Also allows for using different control key profiles based on the OS in use, so can make things more mac-y for example
...
class windowIcon(wx.TaskBarIcon):
    """Creates the icon displayed on the """
...
class MainWindow(wx.Frame):
    ...
    def __init__(self,parent,id,title, windowsize=(800,600)):
        ...
        # Initialise image rescources (icons etc.) this needs changing to use a img2py rescource file
        self.imres = tc.ImRes()

        # Produce the taskbar icon
        self.icon = imres.get
...
##        self.update()       # Init this control with the default values from the active project
    ...
    def translate(self):
        ...
        self.menu_file_new.SetItemLabel(gtm("&New Project", "Ctrl-N"))
        self.menu_file_new.SetHelp(gt("tt_menu_file_new"))
        self.menu_file_open.SetItemLabel(gtm("&Open Project", "Ctrl-O"))
        self.menu_file_open.SetHelp(gt("tt_menu_file_open"))
        self.menu_file_save.SetItemLabel(gtm("&Save Project", "Ctrl-S"))
        self.menu_file_save.SetHelp(gt("tt_menu_file_save"))
        self.menu_file_saveas.SetItemLabel(gtm("Save Project &As", "Ctrl-A"))
        self.menu_file_saveas.SetHelp(gt("tt_menu_file_saveas"))
        self.menu_file_cut.SetItemLabel(gtm("&Cut Image", "Ctrl-K"))
        self.menu_file_cut.SetHelp(gt("tt_menu_file_cut"))
        self.menu_file_export.SetItemLabel(gtm("&Export .pak", "Ctrl-E"))
        self.menu_file_export.SetHelp(gt("tt_menu_file_export"))
        self.menu_file_exit.SetItemLabel(gtm("E&xit", "Alt-Q"))
        self.menu_file_exit.SetHelp(gt("tt_menu_file_exit"))
        # Tools menu
        self.menu.SetMenuLabel(1,"&Tools")
        self.menu_tools_dat.SetItemLabel(gtm(".&dat file options", "Ctrl-D"))
        self.menu_tools_dat.SetHelp(gt("tt_menu_tools_dat"))
        self.menu_tools_smoke.SetItemLabel(gtm("&Smoke options", "Ctrl-M"))
        self.menu_tools_smoke.SetHelp(gt("tt_menu_tools_smoke"))
        self.menu_tools_prefs.SetItemLabel(gtm("&Preferences...", "Ctrl-P"))
        self.menu_tools_prefs.SetHelp(gt("tt_menu_tools_prefs"))
        # Help menu
        self.menu.SetMenuLabel(2,"&Help")
        self.menu_help_help.SetItemLabel(gtm("TileCutter Help", ""))
        self.menu_help_help.SetHelp(gt("tt_menu_help_help"))
        self.menu_help_about.SetItemLabel(gtm("&About TileCutter", ""))
...
# Might be worth looking into a way to select keybinding systems based on user preference/keymapping rather than
# based on the specific translation used though
