...
class Translator:
    ...
    def setActiveTranslation(self, name):
        ...
        # Selection box in GUI uses translated names, these need to be mapped to the names used
        # Dict, longname2name converts from menu names to lookup names
        # Second dict, translations, lookup based on name to return project
        # Active translation is an alias
...
class translation:
    ...
    def icon(self, value=None):
        ...
        if value != None:
##            try:
            self.value_icon_image = wx.Image(PATH_TO_TRANSLATIONS + os.path.sep + value)
            self.value_icon = self.value_icon_image.ConvertToBitmap()
##            except:
##                debug("Icon setting failed, reverting to default")
