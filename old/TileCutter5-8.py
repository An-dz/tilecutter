...
##        self.buffer.SetHeight(bitmap.GetHeight())
        ...
        dc.Clear()
        # Draw the active image aligned to the bottom of the mask
        ...
        if bitmap.GetHeight() < mask_height + mask_offset_y:
            ...
            bmp_offset_y = 0
        debug("Bitmap offset X: %s, Y: %s" % (str(bmp_offset_x), str(bmp_offset_y)))
        ...
        for xx in range(1,x+1):
            ...
            debug("Xpos %i: %s" % (xx, str(pos)))
            if xx == x:
                # Draw left bit for leftmost one & upper left bit
                dc.DrawLine(pos[0],pos[1]-p4,           pos[0],pos[1]-p*z)
            else:
                # Else just draw the upper left bit
        ...
        for yy in range(1,y+1):
            ...
            debug("Ypos %i: %s" % (xx, str(pos)))
            if yy == y:
                # Draw right bit for rightmost one & upper right bit
                # Values of p in x here are -1 because of the way that lines are drawn
                dc.DrawLine(pos[0]+(p-1),pos[1]-p4,     pos[0]+(p-1),pos[1]-p*z)
            else:
                # Else just draw the upper right bit
