...
# Could also encase entire script in an exception catcher, which can display exception and then gracefully
#   shutdown wx, to prevent the flashing box/pythonwin crashing problem
...
# Merge imagePathControl into ImageWindow, as both will be updated together by other controls
...
class facingControl(wx.StaticBox):
    ...
    def OnSouth(self,e):
        ...
        activeproject.activeImage(direction=South)
        # Redraw active image                   - changing the active image must also update the path entry control
        #                                       - Merge impath entry control with the DC??
    def OnEast(self,e):
...
##        self.update()       # Init this control with the default values from the active project
    ...
    def OnUp(self,e):
        """Move mask up"""
        if self.offset_selector.GetValue():
            activeproject.offset(y=1)
        else:
            activeproject.offset(y=activeproject.paksize())
    def OnLeft(self,e):
        """Move mask left"""
        if self.offset_selector.GetValue():
            activeproject.offset(x=-1)
        else:
            activeproject.offset(x=-activeproject.paksize())
    def OnCenter(self,e):
        """Reset mask position"""
        activeproject.offset(x=0, y=0)
    def OnRight(self,e):
        """Move mask right"""
        if self.offset_selector.GetValue():
            activeproject.offset(x=1)
